...
export const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case REMOVE_LOADING: {
      const newLoadingMessages = state.loadingMessages.filter(
        (loadingMessage, i, arr) =>
          arr.indexOf(loadingMessage) !== i ||
          loadingMessage !== action.payload.message,
      );
      return {
        ...state,
        ...
        loadingMessage:
          newLoadingMessages.length === 0
            ? initialState.loadingMessage
            : newLoadingMessages[newLoadingMessages.length - 1],
      };
    }
    case REMOVE_ERROR:
      return {
